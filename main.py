...
POOL_SIZE = (2, 2)
VERBOSE = 0
TEST_MODEL_DIR = 'models/' + MODEL_NAME

TEST_DCM_FILES = ['data/axial/abdomen/2-CHEST  2.0  B30f  SOFT TISSUE-76484/000131.dcm',
                  'data/axial/abdomen/11-Opoznione  1.5  3D-69882/000072.dcm',
                  'data/axial/chest/3-CHEST  3.0  I70f  2-63964/000015.dcm']

# TEST_DCM_FILES = ['data/coronal_sagittal/chest/601-Coronal chest-85685/000031.dcm',
#                   'data/coronal_sagittal/abdomen_chest/80252-COR-46881/000052.dcm',
#                   'data/coronal_sagittal/abdomen_chest_pelvis/602-Cor 2x2-83556/000033.dcm']

# TEST_DCM_FILES = [get_random_dcm('data/axial/abdomen/'),
#                   get_random_dcm('data/axial/abdomen/'),
...
"""
    MODE          (str): train, test 
    MODEL_NAME    (str): MobileNet_V2, MobileNet_V2_aug, ResNet_V2, Inception_V3, Inception_ResNet_v2, EfficientNet_B7, Custom_V1
...
parser.add_argument('-tmd', '--test_model_dir', metavar='',
                    default='models/' + parser.parse_args().model_name, type=str,
                    help='model directory for testing mode')
...
class Net:
    ...
    def get_conv_block(self, input_layer, level):
        ...
        # output = layers.Dropout(rate=0.05*(level+1))(output)
        return output
    ...
    def test_model(self, dcm_paths, thresh=0.5):
        ...
        for dcm_path in dcm_paths:
            ...
            img_source = tfio.image.decode_dicom_image(img_bytes, color_dim=False, dtype=tf.float64, scale='auto')
            img_resized = tf.image.resize(images=img_source, size=(config['img_size']['value'][0],
                                                                   config['img_size']['value'][1]))
            if args.img_channels == 1:
                img_out = tf.identity(img_resized)
            else:
                img_out = tf.image.grayscale_to_rgb(img_resized, name=None)
            ...
            end_reg = time.time()
            pred_times.append(end_reg-start_reg)
            ...
            if args.is_tta:
                ...
                img_out_tta = tf.keras.backend.get_value(img_out)
                if args.test_model_dir == 'models/Custom_V1':
                    raise ValueError('TTA prediction for Custom_V1 model is temporarily not working!')
                start_tta = time.time()
                model_probs_tta = tta_predictor.predict_images(img_out_tta)
                end_tta = time.time()
            ...
            if args.verbose == 0:
                ...
                print('Prediction time............: {:1.3f} seconds'.format(end_reg - start_reg))
                print('Prediction time(TTA).......: {:1.3f} seconds'.format(end_tta - start_tta)) if args.is_tta else ''
                print('-' * 100)
            elif args.verbose == 1:
                style.use('default')
                plt.figure(dcm_path, figsize=(6, 9))
                plt.imshow(np.squeeze(img_source.numpy()), cmap='gray')
                if args.is_tta:
        ...
        if len(args.test_dcm_files) > 1:
            del pred_times[0]
        print('Average prediction time....: {:1.3f} seconds'.format(sum(pred_times) / len(pred_times)))
        print('Average FPS................: {:1.2f} frames per second'.format(1./(sum(pred_times) / len(pred_times))))
        print('-' * 100)
